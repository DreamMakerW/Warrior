...
void AWarriorHeroCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	// subsystem should not be null
	check(Subsystem);
	Subsystem->AddMappingContext(InputConfigDataAsset->DefaultMappingContext, 0);
	
	// 2.把input action输入到callback function
	...
	// 把所有的有效的FWarriorInputActionConfig遍历一遍，进行tag、action与回调函数的绑定
	WarriorInputComponent->BindAbilityInputAction(InputConfigDataAsset, this, &ThisClass::Input_AbilityInputPressed, &ThisClass::Input_AbilityInputReleased);
}

void AWarriorHeroCharacter::BeginPlay()
{
	// 如果不调用Super::BeginPlay()，否则此BeginPlay()函数将无法正常工作
	Super::BeginPlay();
	// Debug::Print(TEXT("Debug helper is working!"));
}
